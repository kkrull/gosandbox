package minimax

type Scorer struct {
	Maximizer Player
	Minimizer Player
}

func (scorer *Scorer) Score(game GameState, player Player) int {
	if player == scorer.Maximizer {
		return scorer.negamax(game, 1)
	} else {
		return reverseThePolarity(scorer.negamax(game, -1))
	}
}

func (scorer *Scorer) negamax(game GameState, polarity int) int {
	if game.FindWinner() == scorer.Maximizer {
		return 1 * polarity
	} else if game.FindWinner() == scorer.Minimizer {
		return reverseThePolarity(polarity)
	} else if game.IsOver() {
		return 0
	}

	maxScore := -100
	for _, nextMove := range game.AvailableMoves() {
		nextGame := game.Move(nextMove)
		nextScore := reverseThePolarity(scorer.negamax(nextGame, reverseThePolarity(polarity)))
		if nextScore > maxScore {
			maxScore = nextScore
		}
	}

	return maxScore
}

func reverseThePolarity(polarity int) int {
	return -1 * polarity
}

type GameState interface {
	AvailableMoves() []Move
	FindWinner() Player
	IsOver() bool
	Move(move Move) GameState
}

type Move string
type Player string
