...
var _ = Describe("Scorer", func() {
	...
	Describe("#Score", func() {
		...
		Context("given a game with 2 or more moves left", func() {
			...
			It("the maximizer assumes the minimizer picked the move with lowest score", func() {
				Expect(scorer.Score(game, max)).To(Equal(0))
			})

			It("the minimizer assumes the maximizer picked the move with highest score", func() {
