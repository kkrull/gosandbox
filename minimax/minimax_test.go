...
var _ = Describe("Scorer", func() {
	...
	Describe("#Score", func() {
		...
		Context("given a game with 2 or moves left", func() {
			...
			It("the maximizer assumes the minimizer will pick the move with the lowest score", func() {
				Expect(scorer.Score(game, max)).To(Equal(0))
			})

			It("the minimizer assumes the maximizer will pick the move with the highest score", func() {
