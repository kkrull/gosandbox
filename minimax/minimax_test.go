...
var _ bool = Describe("Scorer", func() {
	var (
	...
	Describe("#Score", func() {
		...
		Context("given a game with 2 or moves left", func() {
			...
			It("the maximizer assumes the minimizer picks the moves with the lowest score", func() {
				Expect(scorer.Score(game, max)).To(Equal(0))
			})

			It("the minimizer assumes the maximizer picks the moves with the highest score", func() {
